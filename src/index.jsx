...
export default class NativeMenu extends React.Component {
  ...
  item = item => {
    if (this.props.show && this.props.show.includes(item)) {
      const [name, secondary, disabled] = DefaultItems(item, this.state.selection)
      return this.menuItem(name, secondary, disabled, false, e =>
  ...
  menuItem = (primary, secondary, disabled, showSelection, onClick) => {
    if (typeof onClick !== 'function') onClick = new Function()
    const { classes } = this.state
    return (
      <MenuItem onClick={onClick.bind(this)} disabled={!!disabled}>
        <div className={classes.primary}>{primary}{showSelection === true ? ` "${this.state.selection}"` : ''}</div>
          ...
          {image ? (
            ...
            <React.Fragment>
              ...
              {!this.props.minimal &&
                (config.mimic)}
              {customItems}
              {customItems && !this.props.minimal}
              {!this.props.minimal && this.item('view-source')}
            </React.Fragment>
          ) : (
                  <React.Fragment>
